...
char* gettime(tipo_processo *ptr){
    ...
    snprintf(str, 28, "%s", asctime(localtime(&ptr->tempo))); //armazena o valor uma string com a data em str
    return str;
}
void setprioridade(tipo_processo *ptr) {
    //Não posso chamar srand aqui
...
void setPID(tipo_processo *ptr) {
    ...
    //prioridade gerada aleatoriamente
}
...
struct tipo_processo * initprocesso(){
    //Função para iniciar um tipo_processo aloca e seta os valores não necessariamente será utilizada
    tipo_processo *ptr;
